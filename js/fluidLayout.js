...
var FluidLayout = {
    ...
    allTiles: function(reset) {
        ...
        var containerWidth = $(this.displayOptions.container).width();
        var columns = Math.floor((containerWidth + this.displayOptions.offset) / columnWidth);
    ...
    updateTileColumns: function(columnWidth, reset) {
        // Loop over items.
        var item, i=0, k=0, length=$(this.displayOptions.container).find(this.displayOptions.itemClass).length, shortest=null, shortestIndex=null;
        for(; i < length; i++ ) {
